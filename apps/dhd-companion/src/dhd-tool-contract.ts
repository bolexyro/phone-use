...
const DHD_TOOL_DESCRIPTIONS: Record<DhdToolName, string> = {
  ...
  dhd_get_foreground_app:
    "Reports the Android package, activity, and display context in the foreground of this task's virtual display. This tool is read-only: it does not create an observation ID or authorize an action. Open an app with dhd_open_app first if the task has no display yet. Use dhd_observe before sending phone input unless a preceding action already returned a fresh observation.",
  dhd_observe:
    `Captures this task's virtual display and returns its screenshot, foreground context, display details, and a new observation ID. Open an app with dhd_open_app first if the task has no display yet. Use this when no usable observation is available, after an observation-related failure, or when the screen may have changed independently. Do not call it repeatedly for an unchanged screen or immediately after a successful dhd_open_app, dhd_execute, or dhd_execute_sequence; those tools already return a fresh observation. ${SCREENSHOT_MARKER_GUIDANCE}`,
  dhd_open_app:
