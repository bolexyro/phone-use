...
const DHD_TOOL_DESCRIPTIONS: Record<DhdToolName, string> = {
  ...
  dhd_get_foreground_app:
    "Reports the Android package, activity, display context, and screenProtection metadata in the foreground of this task's virtual display. This tool is read-only: it does not create an observation ID or authorize an action. Open an app with dhd_open_app first if the task has no display yet. Use dhd_observe before sending phone input unless a preceding action already returned a fresh observation.",
  dhd_observe:
    `Captures this task's virtual display and returns its screenshot, foreground context, display details, screenProtection metadata, and a new observation ID. If screenProtection.requiresUserAttention is true, call dhd_request_attention and wait for the user's Done acknowledgement before sending more input. Open an app with dhd_open_app first if the task has no display yet. Use this when no usable observation is available, after an observation-related failure, or when the screen may have changed independently. Do not call it repeatedly for an unchanged screen or immediately after a successful dhd_open_app, dhd_execute, or dhd_execute_sequence; those tools already return a fresh observation. ${SCREENSHOT_MARKER_GUIDANCE}`,
  dhd_open_app:
    `Launches one Android app on this task's separate virtual display, creating that display if needed, without requiring a caller-supplied observation ID. On success, it returns the resulting screenshot and a fresh observation ID. In restricted mode, the requested package must be explicitly allowed; Full Access permits any launchable app. Inspect and reuse the returned observation for the next action unless the screen may have changed after the returned capture. ${SCREENSHOT_MARKER_GUIDANCE}`,
  dhd_execute:
  ...
  dhd_request_attention:
    "Blocks the Codex turn until the user reviews the phone and taps Done in DHD. Use this when observation.screenProtection.requiresUserAttention is true, when the task preview is blank because of protected content, or when the app requires a biometric, PIN, passcode, or other user-only step. The task display stays alive while waiting. Never guess or send typed input for biometric/PIN authentication. After this tool returns, use its fresh observation when present or call dhd_observe before continuing.",
};
